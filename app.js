...
class Player {
  ...
  joinTeam() {
    ...
    io.to(this.id).emit('yourTeam', this.team)
  }
}
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('switchPlayerMode', (data) => {
    if (!PLAYER_LIST[socket.id]) return // Prevent Crash
    let room = PLAYER_LIST[socket.id].room        // Get room the client was in
    ROOM_LIST[room].playerMode = data.playerMode  // Update the rooms difficulty
    ROOM_LIST[room].game.playerMode = data.playerMode  // Update the rooms difficulty
    gameUpdate(room)                              // Update the game for everyone in this room
  })
...
function gameUpdate(room) {
  ...
  let gameState = {
    ...
    playerMode: ROOM_LIST[room].playerMode,
  }
