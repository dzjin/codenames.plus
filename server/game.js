...
class Game {
  constructor() {
    ...
    this.playerMode = 'normal' // alternatively 'duet'
  ...
  checkWin() {
    ...
    if (this.playerMode == 'duet') {
      if (this.red == 0 && this.blue == 0) {
        this.over = true
        this.winner = 'everybody'
      }
    }
  }
  ...
  flipTile(i, j) {
    if (!this.board[i][j].flipped) {
      ...
      if (type.includes('death')) { // If death was flipped, end the game and find winner
      ...
      else if (!type.startsWith(this.turn)) this.switchTurn() // Switch turn if opposite teams tile was flipped
  ...
  findType(type) {
    ...
    for (let i = 0; i < 5; i++) {
      for (let j = 0; j < 5; j++) {
        ...
        if (this.playerMode == 'duet') {
          if ((this.board[i][j].type == type || this.board[i][j].type == type + '-shared') && !this.board[i][j].flipped) count++
        }
      }
    }
    return count
  ...
  initBoard() {
    ...
    const soloDeathLimit = 4;
    const sharedLimit = 6;
    const soloNeutralLimit = 2;
    let shared = 0;
    let soloDeath = 0;
    let soloNeutral = 0;
    let color = this.turn;        // First teams color
    const turnCount = this.playerMode == 'duet' ? 17 : 17; // duet is 15 turns
    let duetFinished = false;
    for (let i = 0; i < turnCount; i++) { // Set tiles' color 17 times(9 for team1, 8 for team2)
      ...
      if (this.playerMode == 'duet' && duetFinished == false) {
        if (shared < sharedLimit) {
          this.board[tile.i][tile.j].type = color + '-shared'
          shared++;
        } else if (soloDeath < soloDeathLimit) {

          this.board[tile.i][tile.j].type = color + '-death'
          if (soloNeutral < soloNeutralLimit) {
            this.board[tile.i][tile.j].hiddenNeutral = otherTeam(color);
            soloNeutral++;
          }
          soloDeath++;
        } else if (soloDeath == soloDeathLimit && sharedLimit == sharedLimit) {
          duetFinished = true;
        }
      }
